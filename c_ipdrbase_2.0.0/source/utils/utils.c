				  ...
				  )
{
		...
		if ((pTimeMicroSec == NULL) || (timeStr == NULL)) {
	    /* Returns Appropriate Error Structure*/
			*pErrorCode = IPDR_NULL_INPUT_PARAMETER;
		    return (IPDR_WARNING);
		}
		...
		tempStr = (char*) calloc(100, sizeof(char));
		sprintf(tempStr, "%.lf", (double)*pTimeMicroSec);
		...
		microSeconds = atoi(temp);


		TimeSec1 = ((double)(*pTimeMicroSec) - microSeconds)/1000000;
		TimeSec = (long)TimeSec1;
		newTime = gmtime( &TimeSec );
		...
		*/
		...
		if(microSeconds == 0) {
				strcat(timeStr, "000000");
		}
		else {
				sprintf(microSec, "%d", microSeconds);

				if ((microSeconds >0 ) && (microSeconds <10)){

						strcat(timeStr, "00000");
						strcat(timeStr, microSec);
					}
				else if ((microSeconds >=10) && (microSeconds <100)){
						strcat(timeStr, "0000");
						strcat(timeStr, microSec);
				}
				else if ((microSeconds >=100) && (microSeconds <1000)){
						strcat(timeStr, "000");
						strcat(timeStr, microSec);
				}
				else if ((microSeconds >=1000) && (microSeconds <10000)){
						strcat(timeStr, "00");
						strcat(timeStr, microSec);
				}
				else if ((microSeconds >=10000) && (microSeconds <100000)){
						strcat(timeStr, "0");
						strcat(timeStr, microSec);
				}
				else {
				  ...
				  )
{
	...
	if (( timeStr == NULL) || (pTimeMicroSec == NULL )){
	/* Returns Appropriate Error Structure*/
		*pErrorCode = IPDR_NULL_INPUT_PARAMETER;
	     return (IPDR_WARNING);
	 }
   ...
   /*! Calculating no. of days*/
	...
	subString(timeStr, strMin, 14, 2);
	subString(timeStr, strSec, 17, 2);
	subString(timeStr, strMicro, 20, 6);
	...
	if (strcmp(strMicro, "") == 0) strcpy(strMicro, "0");   

	totalDays = daysInYear + daysInMnth + atoi(strDay) - 1;
			                                           
	*pTimeMicroSec = (hyper)(atoi(strMicro) + (hyper)((atoi(strSec) + 
		               (hyper)((atoi(strMin) + (hyper)((atoi(strHour) + 
					                    totalDays*24)*60))*60))*1000*1000));
						...
						)
{
	...
	if ((strIPAddrSource == NULL) || (byteIPAddr == NULL) ){
	/* Returns Appropriate Error Structure*/
		return (IPDR_NULL_INPUT_PARAMETER);
	}
	...
	/*!
	 * This function reads the strIPAddr and converts it into byte array.
	 */
	for (counter=0, count=0; strIPAddrSource[counter] != '\0'; counter++, 
		                                                    count++) {


		 if(((int)(strIPAddrSource[counter]) <= 57) && ((int)
			                   (strIPAddrSource[counter]) >= 48))	{
			 tempHex =(strIPAddrSource[counter]) - 48;

		 } 
		 	 
		 else {
			tempHex = toupper(strIPAddrSource[counter]) - 55;
		 }
		 
		 counter++;

	     if(((int)(strIPAddrSource[counter]) <= 57) && 
					((int) (strIPAddrSource[counter]) >= 48))	{
		    tempHex1 = (strIPAddrSource[counter]) - 48;
		 } 
		
		 else {
			tempHex1 = toupper(strIPAddrSource[counter]) - 55;
		 }
		...
		byteIPAddr[count]=byte;	 
	}

	byteIPAddr[count] = '\0';

    /*padding with 0 if length less than 32 */
/*
    if((strlen(byteIPAddr)%4)!=0) {
       for(count=(strlen(byteIPAddr)%4);count<4;count++)
         strcat(byteIPAddr, "0");
    }
*/
/*
    if ((strlen(byteIPAddr) < 32)) {
       for (count = 0; count < (32 - strlen(byteIPAddr)); count++ )
          byteIPAddr = strcat("0", byteIPAddr);
    }
*/
							...
							)
{
	...
	if ((byteIPAddr == NULL) || (strIPAddr == NULL)) {
	/* Returns Appropriate Error Structure*/
		return (IPDR_NULL_INPUT_PARAMETER);
	}
	...
	for (counter=0; counter<16; counter++) {
	    ...
	    tempInt = (int)(byteIPAddr[counter]);
		
        sprintf(tempHex, "%x", tempInt);
        strcat(strIPAddr,tempHex);
	}

   /* Adding code to unpad leading zeroes */
   for (counter = 0; counter < strlen(byteIPAddr); counter ++)
   {
       if (byteIPAddr[counter] == '0')
           byteIPAddr = byteIPAddr + 1;
       else
           break;
   }
...
int convAttributeTypeToTypeCode(char* AttributeType)
{
	...
	if (strcmp("INTEGER", tempAttributeType) == 0) {
		...
		free(tempAttributeType);
		return (IPDRDATA_HEXADECARRAY);
	}

	else if (strcmp("UTF8_ARRAY", tempAttributeType) == 0) {
		free(tempAttributeType);
		return (IPDRDATA_STRING);
	}

	else if (strcmp("BOOLEAN", tempAttributeType) == 0) {
		free(tempAttributeType);
		return (IPDRDATA_BOOLEAN);
	}

	else if (strcmp("SHORT", tempAttributeType) == 0) {
		free(tempAttributeType);
		return (IPDRDATA_SHORT);
	}

	else if (strcmp("UNSIGNED_SHORT", tempAttributeType) == 0) {
		free(tempAttributeType);
		return (IPDRDATA_USHORT);
	}

	else if (strcmp("BYTE", tempAttributeType) == 0) {
		free(tempAttributeType);
		return(IPDRDATA_BYTE);
	}

	else if (strcmp("UNSIGNED_BYTE", tempAttributeType) == 0) {
		free(tempAttributeType);
		return(IPDRDATA_UBYTE);
	}
...
int convTypeCodeToAttributeType(int typeCode, char* attributeType)
{
	...
	if (typeCode == IPDRDATA_LONG) 
		strcpy(attributeType, "INTEGER");

	else if (typeCode == IPDRDATA_ULONG) 
		strcpy(attributeType, "UNSIGNED_INTEGER");

	else if (typeCode == IPDRDATA_LONGLONG) 
		strcpy(attributeType, "HYPER");

	else if (typeCode == IPDRDATA_ULONGLONG) 
		strcpy(attributeType, "UNSIGNED_HYPER");

	else if (typeCode == IPDRDATA_FLOAT) 
		strcpy(attributeType, "FLOAT");

	else if (typeCode == IPDRDATA_DOUBLE) 
		strcpy(attributeType, "DOUBLE");

	else if (typeCode == IPDRDATA_HEXADECARRAY) 
		strcpy(attributeType, "BYTE_ARRAY");
	
	else if(typeCode == IPDRDATA_STRING) 
		strcpy(attributeType, "UTF8_ARRAY");

	else if (typeCode == IPDRDATA_BOOLEAN) 
		strcpy(attributeType, "BOOLEAN");

	else if (typeCode == IPDRDATA_SHORT) 
		strcpy(attributeType, "SHORT");

	else if (typeCode == IPDRDATA_USHORT) 
		strcpy(attributeType, "UNSIGNED_SHORT");
	
	else if (typeCode == IPDRDATA_BYTE) 
		strcpy(attributeType, "BYTE");

	else if (typeCode == IPDRDATA_UBYTE) 
		strcpy(attributeType, "UNSIGNED_BYTE");
...
int serializeFNFTypeStructure(FNFType* pFNFType, char* result, int* pErrorCode)
{
	...
	while(pCurrent->pListAttributeDescriptor_ != NULL)
	{
            ...
            strlen(pCurrent->pListAttributeDescriptor_->pAttributeDescriptor_->description_) +  
	    		...
	    		strlen(pCurrent->pListAttributeDescriptor_->pAttributeDescriptor_->complexType_) +
				sizeof(pCurrent->pListAttributeDescriptor_->pAttributeDescriptor_->noOfElements_);
	    tempString = (char *) calloc (lenString, sizeof(char));
		sprintf(tempString, "%s*%s*%s*%s*%s*%d$", 
            ...
            pCurrent->pListAttributeDescriptor_->pAttributeDescriptor_->description_, 
	    		...
	    		pCurrent->pListAttributeDescriptor_->pAttributeDescriptor_->complexType_,
				pCurrent->pListAttributeDescriptor_->pAttributeDescriptor_->noOfElements_);
	...
	while(pCurrent != NULL)
	{
                lengthStr = 10 +
                            ...
                            strlen(pCurrent->pAttributeDescriptor_->description_) + 
	    							 ...
	    							 strlen(pCurrent->pAttributeDescriptor_->complexType_) +
									 sizeof(pCurrent->pAttributeDescriptor_->noOfElements_);
	...
	for(pos = pos + 1; inputString[pos] != '\0'; pos++) {
		if (inputString[pos] != '$') {
			if ( inputString[pos] != '*') {
				...
				if (posFlag == 5) {
					strcpy(pDescriptorNode->complexType_  , tempString);
					strcpy(tempString, "");
				}
				posFlag++;
			}
			...
			if (posFlag == 6) {
				pDescriptorNode->noOfElements_ = atoi(tempString);
				strcpy(tempString, "");
			}
			posFlag = 1;
			appendListAttributeDescriptor(
				 ...
				 pDescriptorNode->complexType_,
				 pDescriptorNode->pListAttrNameList_,
				 pDescriptorNode->pListAttrClassList_,
				 pDescriptorNode->pListAttrMaxOccursList_,
				 pDescriptorNode->noOfElements_
				 );
		}
	}
...
int getLengthSerializeFNFType(FNFType* pFNFType)
{
 ...
 if (pFNFType->pListAttributeDescriptor_ != NULL) {
  ...
  while (pCurrent != NULL) {
   ...
   lenSerializeString = lenSerializeString +
    strlen(pCurrent->pAttributeDescriptor_->complexType_) + 1;
	lenSerializeString = lenSerializeString + sizeof(pCurrent->pAttributeDescriptor_->noOfElements_);
 
   /* Include No of delimeters to be added for one AttributeDescriptor */
	/*include noOfelements also */
   lenSerializeString = lenSerializeString + 6;
