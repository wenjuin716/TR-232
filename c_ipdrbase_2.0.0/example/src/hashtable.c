...
void deallocateKey (void* key);
void deallocateValue (void* value);
						  ...
						  ) 
{
	...
	HashTableSetDeallocationFunctions (hashTable, deallocateKey, deallocateValue);
    return hashTable;
}
 ...
 *      O(n)
 *  ARGUMENTS:
 *      hashTable    - the HashTable to de``stroy
 *  RETURNS:
 *      <nothing>
*/
    ...
    for (i=0; i<hashTable->numOfBuckets; i++) {
        ...
        while (pair != NULL) {
            ...
            if (hashTable->valueDeallocator != NULL)
                hashTable->valueDeallocator((void *) pair->value);
			free(pair);
            pair = nextPair;
        }
    }
				...
				) 
{
    ...
    if (pair) {
        ...
        KeyValuePair *pairNew = (KeyValuePair *) malloc(sizeof(KeyValuePair));
        if (pairNew == NULL) {
            return -1;
        }
        else {
			pairNew->key = (char*) calloc(sizeof(char), strlen(key)+1);
            strcpy((char *)pairNew->key,(char *)key);
			pairNew->value = (char*) calloc(sizeof(char), 10);
			sprintf(pairNew->value , "%d" , (int *) value);
            pairNew->next = hashTable->bucketArray[hashValue];
            hashTable->bucketArray[hashValue] = pairNew;
...
/*!
 *  NAME:
 *      HashTablePutstr() - adds a key/value pair to a HashTable
 *  DESCRIPTION:
 *      Adds the specified key/value pair to the specified HashTable.  If
 *      the key already exists in the HashTable (determined by the comparison
 *      function specified by HashTableSetKeyComparisonFunction()), its value
					...
					) 
{
    ...
    if (pair) {
        ...
        if (pairNew == NULL) {
			...
			pairNew->key = (char*) calloc(sizeof(char), strlen(key) + 1);
			strcpy((char *) pairNew->key,(char *) key);
			pairNew->value = (char*) calloc(sizeof(char), strlen(value) + 1);
			sprintf(pairNew->value , "%s" , (char *) value);
            pairNew->next = hashTable->bucketArray[hashValue];
            hashTable->bucketArray[hashValue] = pairNew;
            hashTable->numOfElements++;
...
/*!
 *  NAME:
 *      pointercmp() - Comparing two pointer values.
 *  DESCRIPTION:
 *      This fucntion compares the vales of two input pointers.
 *
 *  ARGUMENTS:
 *      pointer1  - The pointer to be compared.
 *      pointer2  - Second pointer to be compared.
 *  RETURNS:
 *      int - The output of string compare function.
 */
...
/*!
 *  NAME:
 *      pointerHashFunction() - Puts pointer as input to hash fucntion.
 *  DESCRIPTION:
 *      Puts pointer as input to hash fucntion..
 *
 *  ARGUMENTS:
 *      pointer  - The pointer to be put to hash function.
 *
 *  RETURNS:
 *      long - Hash function of the input pointer value..
 */
...
static unsigned long pointerHashFunction(const void *pointer) 
{
	char randStr[1000];
	int pos=0;
	unsigned long randAscii=0;
	unsigned long randHash=0;
	strcpy(randStr,pointer);
	for(pos=0;  randStr[pos] != '\0'; pos++) {
		randAscii = randAscii + randStr[pos];
	}
        randHash=randAscii >> 4;
        return (randHash);
...
/*!
 *  NAME:
 *      isProbablePrime() - Checks for a number being prime.
 *
 *  DESCRIPTION:
 *      Puts pointer as input to hash fucntion.
 *
 *  ARGUMENTS:
 *      oddNumber - The pointer to be put to hash function.
 *
 *  RETURNS:
 *      int - 1 if nos is prime else 0.
 */
...
/*!
 *  NAME:
 *      calculateIdealNumOfBuckets() - Calculates the ideal no of buckets.
 *
 *  DESCRIPTION:
 *      Finds the number of buckets from a given hash table.
 *
 *  ARGUMENTS:
...
static long calculateIdealNumOfBuckets(HashTable *hashTable) 
{
    long idealNumOfBuckets = hashTable->numOfElements / (long) hashTable->idealRatio;
