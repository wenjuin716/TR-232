...
int printUsage()
{
    ...
    printf("\nCorrect Usage is:");
    printf("\n./testXDR <read/write> <schema Validation ON/OFF> ");  
    printf("\ne.g. ./testXDR write OFF\n");
    return 0;
}
...
int readIPDRDoc(int flag)
{
	...
	if ((streamHandle = fopen("IPDRXDRDoc.xdr", "rb+")) == NULL) {
	...
	while(pFNFData != NULL) {
		...
		numOfElements++;
                if(numOfElements % 100 == 0) printFNFData(pFNFData); 
...
int writeIPDRDoc(int flag )
{
	...
	if ((streamHandle = fopen("IPDRXDRDoc.xdr", "w+")) == NULL) {
		printf("\nError in opening file");
		return -1;
	} 
...
int writeRawIPDRDoc()
{
	...
	/*---------------------------------*/
	/* Populating IPDRCommonParameters */
	/*---------------------------------*/
	initIPDRCommonParameters(
		pIPDRCommonParameters,
		XDR,
	...
	/*-----------------------------------*/
	/* Populating Service Definition URI */
	/*-----------------------------------*/ 
	appendListServiceDefinitionURI(&pListServiceDefinitionURI, 
		"VoIP3.0-A.0ID.xsd");
...
//	appendListAttributeDescriptor(&pFNFData->pListAttributeDescriptor_, "tryUTF8", "UTF8_Array", "", "UTF8_Array","",NULL,NULL);
//	appendListAttributeDescriptor(&pFNFData->pListAttributeDescriptor_, "tryLong", "integer", "", "integer","",NULL,NULL);
//	appendListAttributeDescriptor(&pFNFData->pListAttributeDescriptor_, "try1UTF8", "UTF8_Array", "", "UTF8_Array","",NULL,NULL);
//	appendListAttributeDescriptor(&pFNFData->pListAttributeDescriptor_, "try1Long", "integer", "", "integer","",NULL,NULL);
